...
Play Together 是当前 P0 独有核心和唯一实机/发布主线。2026-08-30 用户进一步授权：
可以并行把已经由固定版本官方 App 闭合的 JBL One 网络与控制行为，以 clean-room 方式
迁移为 Ubuntu Rust 的类型、解析器、合成 fixture、只读命令和默认未启用的研究序列化器。
该 P1 离线迁移不得操作音响、消耗写入验收次数、绕过单写者，或把未实测功能宣传为
已支持。EQ、通用播放、源、按键、模式等生产写入口仍属于 P2，必须逐项通过证据与实机
门槛后才能启用。
...
音量、静音、EQ、播放源与 Home Assistant 不自动进入当前 Play Together 稳定版的
“已支持”范围；但已闭合的网络协议允许在 P1 作为离线、只读或默认未启用的 clean-room
端口实现。任何生产写入口仍需单独需求、合成门槛与真实设备验收。
